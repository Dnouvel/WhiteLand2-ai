import { type Plot, type InsertPlot, type HbuStudy, type InsertHbuStudy, type User, type InsertUser, type DevelopmentScenario, type ZoningDetails, type MarketData, type SpaceProgram, type FinancialSummary } from "@shared/schema";
...
// Sample city plot data for Riyadh, Saudi Arabia (sizes in sqm, values in SAR)
...
// Pre-stored HBU Studies (extracted from OpenAI, stored for demo)
function generateSampleHbuStudy(plotId: string, plot: Plot): HbuStudy {
...
export class MemStorage implements IStorage {
  ...
  constructor() {
    ...
    // Pre-populate with sample HBU studies for each plot
    samplePlots.forEach(plot => {
      const study = generateSampleHbuStudy(plot.id, plot);
